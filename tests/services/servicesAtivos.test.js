...
describe('SERVICE - Lista as ações de um determinado cliente', () => {
  describe('Id do cliente existe no banco de dados', () => {
    ...
    it('Verifica se retorna um array', async () => {
      const assetsFromClient = await serviceAtivos.getAssetsFromOneClientById(
        mock.USER_INFO.clientId,
        );
    ...
    it('Verifica se o objeto retorna as chaves esperadas', async () => {
      ...
      expect(assetsFromClient[0]).to.include.all.keys(
        'clientId', 'assetId', 'price', 'quantityAsset',
        );
    });
  });
