...
const increaseAssetInWallet = async (quantity, clientId, assetId) => {
  ...
  if (addAsset.affectedRows === 0) {
    throw createErrorObj(400, 'Purchase failed! Verify the asset\'s id');
  }
};
...
const decreaseAssetFromWallet = async (quantity, clientId, assetId) => {
  ...
  if (quantity > quantityAsset) {
    throw createErrorObj(400, 'You can not sale more assets than you have!');
  }
  
  // Diminui a quantidade daquele ativo na carteira
  const [decreaseAsset] = await 
  modelInvestimentos.decreaseAssetFromWallet(quantity, clientId, assetId);
  ...
  if (decreaseAsset.affectedRows === 0) {
    throw createErrorObj(400, 'Sale failed! Verify the asset\'s id');
  }
};
