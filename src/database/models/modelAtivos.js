...
const getAllAssets = async () => {
  const [assets] = await connection.execute(
    `SELECT asset_id as assetId, name_asset as nameAsset, price
    FROM heroku_3a2342a6c76f266.assets;`,
    );
    return assets;
};
...
const getAssetById = async (id) => {
    const [asset] = await connection.execute(
      `SELECT asset_id AS assetId, name_asset AS nameAsset,
       quantity_available AS quantityAvailable, price
...
const getAssetsFromOneClientById = async (clientId) => {
  const [clientAssets] = await connection.execute(
    ...
    WHERE t3.asset_id = t2.asset_id AND t2.client_id =  t1.client_id AND t1.client_id = ?;`,
     [clientId],
  );
  return clientAssets;
};
